...
function buildPRBody(siteId: string, apiUrl: string, filePath: string, framework: Framework): string {
  return [
    ...
    ``,
    `**What it does**`,
    `- Splits visitors 50/50 into cohort A or B (sticky per test via \`localStorage\`)`,
    `- Swaps tracked elements with AI-generated challenger variants in real time`,
    `- Records impressions and clicks, deduped per visitor — no PII, no cookies, no third-party services`,
    ``,
    `**What it does NOT do**`,
